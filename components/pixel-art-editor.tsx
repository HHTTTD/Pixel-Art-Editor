...
const getColorCounts = (layers: Layer[]): Map<string, number> => {
  ...
  return (
    <div className="flex h-screen w-full bg-[#0f1115] text-[#a1a1aa] font-sans selection:bg-orange-500/30">
      ...
      >
        ...
        {/* Color Stats Panel */}
        <div className="flex flex-col bg-[#1c1e24] rounded-2xl overflow-hidden border border-white/5 shadow-xl">
          <div className="p-4 border-b border-white/5 flex items-center justify-between bg-[#22242a]">
            <span className="text-sm font-medium text-zinc-300">Color Stats</span>
            <span className="text-xs text-zinc-500">{sortedColors.length} colors</span>
          </div>

          <div className="max-h-48 overflow-y-auto p-2">
            {sortedColors.length === 0 ? (
              <div className="text-xs text-zinc-500 text-center py-4">No colors used yet</div>
            ) : (
              <div className="space-y-1">
                {sortedColors.map(([color, count]) => (
                  ...
                  >
                    <div
                      className="w-5 h-5 rounded-md border border-white/10 flex-shrink-0 group-hover:scale-110 transition-transform"
                      style={{ backgroundColor: color }}
                    />
                    <span className="text-xs text-zinc-400 font-mono flex-1 truncate">{color.toUpperCase()}</span>
                    <span className="text-xs font-medium text-zinc-300 bg-white/5 px-2 py-0.5 rounded">{count}</span>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2 bg-[#1c1e24] px-3 py-1.5 rounded-lg border border-white/5">
              ...
              <Select
                ...
                onValueChange={(value) => changeGridSize(Number(value))}
              >
                <SelectTrigger className="h-auto min-w-0 border-0 bg-transparent px-0 py-0 text-sm text-white font-medium shadow-none focus:ring-0 [&>svg]:text-zinc-400">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent className="bg-[#1c1e24] border-white/10 text-white">
                  <SelectItem value="8" className="text-white focus:bg-white/10 focus:text-white">
                    8x8
                  </SelectItem>
                  <SelectItem value="16" className="text-white focus:bg-white/10 focus:text-white">
                    16x16
                  </SelectItem>
                  <SelectItem value="32" className="text-white focus:bg-white/10 focus:text-white">
                    32x32
                  </SelectItem>
                  <SelectItem value="64" className="text-white focus:bg-white/10 focus:text-white">
                    64x64
                  </SelectItem>
                </SelectContent>
              </Select>
